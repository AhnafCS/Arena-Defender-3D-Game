...
def draw_key(x, z):
    ...
    glRotatef(-90, 1, 0, 0)
    gluCylinder(gluNewQuadric(), 0.1, 0.1, 0.8, 12, 4)
    glPopMatrix()
    glPopMatrix()
...
def draw_weapon_pickups():
    for w in weapon_pickups:
        ...
        if w["type"] == 1: #pistol
            ...
            glPopMatrix()
        elif w["type"] == 3:  #double barrel shotgun
...
def keyboard(key, x, y):
    ...
    if key == b'r' and (game_over or game_won):  # R key to reset after death or winning
        reset_game()
        glutPostRedisplay()
