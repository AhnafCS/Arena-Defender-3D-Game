...
def update_enemies():
    """Simple AI: chase player if close; damage on contact."""
    ...
    for e in enemies:
        ...
        if dist < e["size"]:  # overlapping player
            if e["damage_cooldown"] <= 0:
                player["health"] -= damage  # or whatever damage
                e["damage_cooldown"] = 600  # 60 frames ≈ 1 sec at 60 fps
...
def check_door_transition():
    ...
    if can_exit and key_pos is None and (player["z"] >= half - 1.0) and (-DOOR_WIDTH/2 <= player["x"] <= DOOR_WIDTH/2):
...
def keyboard(key, x, y):
    ...
    if game_over and key == b'r':  # R key to reset after death
        reset_game()
        glutPostRedisplay()
...
def reset_game():
    global player, current_room_index, game_over, player_lying, enemies
    ...
    half = room["size"] - 5
    for e in enemies:
        e["x"] = random.uniform(-half, half)
        e["z"] = random.uniform(-half, half)
        e["dir"] = random.uniform(0, 360)
        e["damage_cooldown"] = 0
...
# =========================
# ADDED: Shooting System (no changes to your functions above)
# =========================
# (this block follows exactly your previously provided shooting code and wrappers)
import time
# Bullet state
bullets = []  # each: {"x","y","z","vx","vz","speed","radius","ttl","color"}
shooting_mode = 1  # 1=fast small, 2=large slow, 3=spread
...
def _update_bullets(dt):
    ...
    for b in bullets:
        ...
        if hit_index is not None:
            enemies.pop(hit_index)  # remove enemy on hit
            # bullet consumed
            continue
        survivors.append(b)
    bullets = survivors
...
def keyboard_with_shooting(key, x, y):
    _orig_keyboard(key, x, y)  # keep all original movement/logic
    ...
    if key == b' ':
        ...
        shooting_mode = 3
    # ensure a redraw if our keys were used
    if fired or key in (b'1', b'2', b'3'):
        glutPostRedisplay()
keyboard = keyboard_with_shooting  # rebind name used by main()
# =========================
# END ADDED SHOOTING SYSTEM
# =========================
# ============================================================
# MY ADDITION (APPENDED): Enemy Health + Bullet Damage Wrappers
# - I DO NOT MODIFY any of your functions above.
# - I only wrap/rebind names so enemies get HP and bullets carry damage.
# ============================================================
# Note: original small helper was present earlier in your pasted code.
# I now override it (safe -> name rebind) to increase HP as requested.
def _enemy_max_health(etype):
    """
    Increased health values so spider takes ~5 large bullets,
    cube_bot ~3 large bullets, rolling_ball ~2 large bullets.
    Large bullet damage (see shoot_with_damage) is 3 per hit.
    """
    if etype == "spider":
        return 15   # needs ~5 large bullets at 3 dmg each
    elif etype == "cube_bot":
        return 9    # needs ~3 large bullets
    elif etype == "rolling_ball":
        return 6    # needs ~2 large bullets
    else:
        return 5
# 1) Ensure spawn_enemies assigns both health and max_health
# (there was a spawn_enemies_with_health wrapper earlier; rebind again to set max too)
_orig_spawn_enemies = spawn_enemies
def spawn_enemies_with_health():
    # call original spawn_enemies (unchanged)
    _orig_spawn_enemies()
    # add health and max_health fields without changing the original function
    for e in enemies:
        # set health to the increased values if not present
        e["max_health"] = _enemy_max_health(e["type"])
        if "health" not in e:
            e["health"] = e["max_health"]
spawn_enemies = spawn_enemies_with_health
# 2) Wrap shoot() to tag bullets with damage depending on shooting_mode
# (your damage mapping remains: large bullet = 3, fast = 2, spread pellet = 1)
_orig_shoot = shoot
def shoot_with_damage():
    # record how many bullets we had, call original shoot, then tag the new ones
    before = len(bullets)
    _orig_shoot()
...
# 3) Replace _update_bullets with a health-aware version (keeps same signature)
...
def _update_bullets_with_health(dt):
    ...
    for b in bullets:
        ...
        survivors.append(b)
    bullets = survivors
# rebind the function name used elsewhere
...
def draw_health_bar_for_enemy(e):
    """
    Draw a small billboarded health bar above enemy e.
    This function uses simple billboarding by rotating on Y to face the camera angle.
    """
...
# Rebind draw_enemies again to draw health bars after enemies and bullets are updated/drawn.
# We rely on _orig_draw_enemies which holds your original enemy-drawing function.
...
# That's it — everything above is appended without changing the original functions' internals.
# The module-level names were rebound (spawn_enemies, shoot, _update_bullets, draw_enemies)
# to the wrappers so the added behavior (health, damage, visible bars) is active.
# =========================
# END OF APPENDED CODE
# =========================

# ============================================================
# APPENDED: SCOREBOARD / HUD (Health, Score, Room/Wave)
# - Adds a 2D HUD overlay without modifying existing functions.
# - Uses wrappers to hook into enemy updates and room transitions.
# ============================================================

# Basic HUD state (player HP static for now; can be wired to gameplay later)
hud = {
